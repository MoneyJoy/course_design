...
import json                    # JSON库，用于生成JSON格式的指令
...
class MqttGateway:
    ...
    def start_client(self):
        ...
        # 创建MQTT客户端实例
        # mqtt.CallbackAPIVersion.VERSION2 指定使用版本2的回调API
    ...
    def on_message(self, client, userdata, msg):
        """
        接收到MQTT消息时的核心处理函数 (已修改以适配自定义格式)

        参数:
            client: MQTT客户端实例（本例中未使用，但保留以符合回调函数签名）
            userdata: 用户数据（本例中未使用，但保留以符合回调函数签名）
            msg: 接收到的消息对象，包含 topic, payload, qos, retain 等属性
        ...
        预期的消息格式 (自定义格式):
        {client_id;temperature;humidity;light_intensity}
        例如: {m2joy;25;60.5;333.3}

        业务逻辑：
        - 当温度超过30°C时，向设备发送"open_fan"指令
        - 当温度低于25°C时，向设备发送"close_fan"指令
        - 当光照低于50时，向设备发送"open_light"指令
        - 当光照高于50时，向设备发送"close_light"指令
        - 使用状态字典跟踪每个设备的状态，避免重复发送相同指令
        """
        try:
            ...
            print(f"❌ 格式错误: 载荷中包含无法转换为数字的字段 -> {msg.payload.decode('utf-8')}")
        except Exception as e:
            # 其他异常的通用处理
            print(f"❌ 处理消息时发生错误: {e}")
    # =========================================================================
